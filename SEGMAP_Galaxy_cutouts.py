...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 12:28:20 2024
...
galaxy_file = fits.open ('J0305M3150_F356W_segmap.fits')#importing fits file of image
#print(galaxy_file.info())
image= galaxy_file[0].data#getting image data
...
num1 = '20'
center = SkyCoord(ra =XX[int(num1)],dec =YY[int(num1)], unit = u.deg) #center position coordinate
size = u.Quantity((101,101),u.pixel) #cutout size 
cutout = Cutout2D(image,center,size, wcs=w ) #Test cutout
plt.imshow(cutout.data,origin = 'lower')
plt.savefig('cutout'+num1+'F356W_Segmap.png')


newheader = cutout.wcs.to_header() #creating new header
fnew = fits.PrimaryHDU(data=cutout.data, header=newheader) # creating new fits file
fnew.writeto("galaxy"+num1+"_F356W_Segmap_field.fits",overwrite=True) #writing cutout to fits file
